...
function get_subCategory_options($conn = null, $hc = array()) {
    $conn = dbconnect();
    $qry = "SELECT product_type_name, product_type_id FROM " . PROD_CATEGORY . " WHERE parent=1";
...
function mainSearch_query($params = '', $subParams = '', $array = array(), $count = false) {
    ...
    $qry_inner = 'SELECT 
                    ...
                    WHERE ' . $params . '
                        ...
                        GROUP BY
                          t.product_id) GROUP BY t.product_attr_val_id) as tbl2 GROUP BY tbl2.id, tbl2.n, tbl2.an ORDER BY product';
    ...
    $count_query_inner = "(
                                ...
                                LEFT JOIN attr_common_flds_ecomc f ON v.attr_id = f.id 
                                WHERE " . $params .
            "(p.subcatid IN (" . $subParams . ")) GROUP BY t.product_id) GROUP BY t.attribute_value_id";
    if (!$count) {
            ...
            if ($j == 0) {
                $html .= '</tbody></table></div><div class="col-md-2 links"><div class="tools-icons text-center">
                                    <ul class="bibliography_links list-unstyled">
                                        ...
                                        <li><a href="' . SITE_URL . '/seemore/' . $productID . '" class="btn btn-default" target="_blank">See More</a></li>
...
function replace_underscore_space($str = '', $duringUpload=FALSE) {

    if (!empty($str)) {
        if (preg_match("/_/", $str)) {
            if(preg_match('/\breference_type\b/', $str)){
                if($duringUpload !== TRUE)
                {
...
function get_attrKeys_by_category($category = 'Bibliography') {
    ...
    try {
        ...
        $q->setFetchMode(PDO::FETCH_ASSOC);
        while ($row = $q->fetch()) {
            $keyData[] = strtolower(replace_underscore_space($row['attribute_name'], TRUE));
        }
...
function get_all_inputtype_fields($product_type=null) {
    $result = array();
    try {
        $conn = dbconnect();
        $qry = "SELECT attr_common_flds_ecomc.attribute_name,attr_common_flds_ecomc.id"
                . " FROM product_type_attribute_key INNER JOIN attr_common_flds_ecomc"
                . " ON (product_type_attribute_key.attribute_id = attr_common_flds_ecomc.id)"
                . " WHERE product_type_attribute_key.p_type_id=:ptype AND"
                . " attr_common_flds_ecomc.field_type IN ('text', 'textarea', 'file', 'date', 'select-multiple')"
                . " ORDER BY attr_common_flds_ecomc.id";
        $q = $conn->prepare($qry);
        $q->bindParam(':ptype', $product_type, PDO::PARAM_INT);
        $q->execute();
        $q->setFetchMode(PDO::FETCH_ASSOC);
        $row = $q->fetchAll();
        foreach ($row AS $k => $v) {
...
function get_input_type_fields($prod_type_id) {
    $result = array();
    try {
        $conn = dbconnect();
        $qry = "SELECT
...
product_type_attribute_key
INNER JOIN attr_common_flds_ecomc ON product_type_attribute_key.attribute_id = attr_common_flds_ecomc.id
where attr_common_flds_ecomc.field_type IN ('text', 'textarea', 'file', 'date', 'select-multiple') AND product_type_attribute_key.p_type_id=:p_type ORDER BY attr_common_flds_ecomc.id
";
        $q = $conn->prepare($qry);
        $q->execute([':p_type' => $prod_type_id]);
        $q->setFetchMode(PDO::FETCH_ASSOC);
        $row = $q->fetchAll();
        foreach ($row AS $k => $v) {
