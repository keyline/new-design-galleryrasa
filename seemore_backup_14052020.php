...
//        print_r(json_decode($imageDetails[0], true));
    ...
    $flag = 0;
    $html .= '<div class="col-md-12"><div class="col-md-10 table-responsive product-expandable">
    ...
    foreach ($dataImage as $key => $bibloTitle) {
        foreach ($bibloTitle as $k => $v) {
            ...
            if (is_array($v)) {
                for ($i = 0; $i < count($v); $i++) {
                    $imageDetailsHTML .= '<li><div class="image-particulars-' . $i . '"><a class="thumbnail" href="' . SITE_URL . '/' . IMGSRC . 'bibliography/' . $v[$i]['name'] . '"> <img class="img-responsive" src="' . SITE_URL . '/product_images/thumbs/' . $v[$i]['name'] . '" alt="bibliography"></a></div></li><br>';
    ...
    foreach ($arrayFlatten as $key => $val) {
        ...
        if (array_key_exists($key, $filtered)) {
            ...
            if (!$flag)
                $html .= '<tr>
                                                <th class="col-sm-3">' . implode("/", $array_keys) . ':' . '</th>
                                                <td>' . implode("/", $vv) . '</td>
                                            </tr>';
            $flag++;
        }else {
            ...
            $html .= '<tr>
                                                                <th class="col-sm-3">' . stripslashes($key) . ':' . '</th>
                                                                <td>' . $val . '</td>
                                                            </tr>';
        }
    }
    $html .= '</tbody></table>
                                    ...
                                    <ul class="list-unstyled">
                                        <li><a href="javascript:void(0);" data-toggle="modal" data-target="#citethis"><i class="btn btn-default" data-toggle="tooltip" title="" data-original-title="Cite this" data-placement="left" onclick="javascript:CiteThis(' . $arrayFlatten['pid'] . ');">Cite This</i></a></li><br>';
//                                        <li><a href="javascript:void(0);" data-toggle="modal" data-target="#pdf-preview"><i class="btn btn-default" data-toggle="tooltip" title="" data-original-title="Preview" data-placement="left" onclick="javascript:PreviewPdf(' . $arrayFlatten['pid'] . ');">Sneak Preview</i></a></li><br>
    ...
    $title = strtok($category, " ");
    $terms = array($title);
    $count = 0;
    //Defining a preg classback function to replace the wrapping
    $html = preg_replace_callback('/\{{2}(.*?)\}{2}/', function($match) use (&$count, $terms) {
