...
class Mpdf implements \Psr\Log\LoggerAwareInterface
{
	...
	protected $scale_cons = Mpdf::SCALE;
	...
	public function __construct(array $config = [])
	{
		...
		// Line width (0.2 mm)
		$this->LineWidth = .567 / $this->scale_cons;
		...
		if (!$default_font_size) {
			...
			$default_font_size = $mmsize * ($this->scale_cons);
		}
	...
	function _setPageSize($format, &$orientation)
	{
		if (is_string($format)) {
			...
			$this->fwPt = $format[0] * $this->scale_cons;
			$this->fhPt = $format[1] * $this->scale_cons;
		}

		$this->fw = $this->fwPt / $this->scale_cons;
		$this->fh = $this->fhPt / $this->scale_cons;
		...
		$this->w = $this->wPt / $this->scale_cons;
		$this->h = $this->hPt / $this->scale_cons;
	}
	...
	function _resizeBackgroundImage($imw, $imh, $cw, $ch, $resize, $repx, $repy, $pba = [], $size = [])
	{
		...
		$cw = $cw * $this->scale_cons;
		$ch = $ch * $this->scale_cons;
	...
	function SetBackground(&$properties, &$maxwidth)
	{
		...
		if (preg_match('/(-moz-)*(repeating-)*(linear|radial)-gradient/', $properties['BACKGROUND-IMAGE'])) {
			...
			if (isset($sizesarray['IMAGE_ID'])) {
				...
				$orig_w = $sizesarray['WIDTH'] * $this->scale_cons;  // in user units i.e. mm
				$orig_h = $sizesarray['HEIGHT'] * $this->scale_cons;  // (using $this->img_dpi)
	...
	function PrintBodyBackgrounds()
	{
		...
		if ($this->bodyBackgroundColor) {
			...
			$s .= sprintf('%.3F %.3F %.3F %.3F re f Q', ($clx * $this->scale_cons), ($cly * $this->scale_cons), $clw * $this->scale_cons, $clh * $this->scale_cons) . "\n";
		}
		...
		if ($this->bodyBackgroundImage) {
			if (isset($this->bodyBackgroundImage['gradient']) && $this->bodyBackgroundImage['gradient'] && preg_match('/(-moz-)*(repeating-)*(linear|radial)-gradient/', $this->bodyBackgroundImage['gradient'])) {
				...
				$s .= sprintf('q /Pattern cs /P%d scn %s %.3F %.3F %.3F %.3F re f Q', $n, $opac, ($clx * $this->scale_cons), ($cly * $this->scale_cons), $clw * $this->scale_cons, $clh * $this->scale_cons) . "\n";
			}
		}
		/* -- END BACKGROUNDS -- */
		return $s;
	...
	function _setClippingPath($clx, $cly, $clw, $clh)
	{
		...
		$s .= sprintf('%.3F %.3F m ', ($clx) * $this->scale_cons, ($this->h - ($cly)) * $this->scale_cons); // start point TL before the arc
		$s .= sprintf('%.3F %.3F l ', ($clx) * $this->scale_cons, ($this->h - ($cly + $clh)) * $this->scale_cons); // line to BL
		$s .= sprintf('%.3F %.3F l ', ($clx + $clw) * $this->scale_cons, ($this->h - ($cly + $clh)) * $this->scale_cons); // line to BR
		$s .= sprintf('%.3F %.3F l ', ($clx + $clw) * $this->scale_cons, ($this->h - ($cly)) * $this->scale_cons); // line to TR
		$s .= sprintf('%.3F %.3F l ', ($clx) * $this->scale_cons, ($this->h - ($cly)) * $this->scale_cons); // line to TL
		...
		foreach ($this->pageBackgrounds as $bl => $pbs) {
			...
			foreach ($pbs as $pb) {
				...
				if ((!isset($pb['image_id']) && !isset($pb['gradient'])) || isset($pb['shadowonly'])) { // Background colour or boxshadow
					...
					$s .= sprintf('%.3F %.3F %.3F %.3F re f Q', $pb['x'] * $this->scale_cons, ($this->h - $pb['y']) * $this->scale_cons, $pb['w'] * $this->scale_cons, -$pb['h'] * $this->scale_cons) . "\n";
			...
			foreach ($pbs as $pb) {
				...
				if (isset($pb['gradient']) && $pb['gradient']) {
					...
					$x = $pb['x'] * $this->scale_cons;
					$y = ($this->h - $pb['y']) * $this->scale_cons;
					$w = $pb['w'] * $this->scale_cons;
					$h = -$pb['h'] * $this->scale_cons;
					...
					if ($this->writingHTMLfooter || $this->writingHTMLheader) { // Write each (tiles) image rather than use as a pattern
						...
						$iw = $pb['orig_w'] / $this->scale_cons;
						$ih = $pb['orig_h'] / $this->scale_cons;
						...
						for ($xi = 0; $xi < $nx; $xi++) {
							for ($yi = 0; $yi < $ny; $yi++) {
								...
								$s .= sprintf("q %s %.3F 0 0 %.3F %.3F %.3F cm /I%d Do Q", $opac, $iw * $this->scale_cons, $ih * $this->scale_cons, $x * $this->scale_cons, ($this->h - ($y + $ih)) * $this->scale_cons, $pb['image_id']) . "\n";
							}
						}
		...
		foreach ($this->tableBackgrounds as $bl => $pbs) {
			foreach ($pbs as $pb) {
				if ((!isset($pb['gradient']) || !$pb['gradient']) && (!isset($pb['image_id']) || !$pb['image_id'])) {
					...
					$s .= sprintf('%.3F %.3F %.3F %.3F re %s Q', $pb['x'] * $this->scale_cons, ($this->h - $pb['y']) * $this->scale_cons, $pb['w'] * $this->scale_cons, -$pb['h'] * $this->scale_cons, 'f') . "\n";
				}
				if (isset($pb['gradient']) && $pb['gradient']) {
					if (isset($pb['clippath']) && $pb['clippath']) {
						$s .= $pb['clippath'] . "\n";
	...
	function AddPage(
		...
		if ($this->state > 0 && $this->page < count($this->pages)) {
			...
			$this->LineWidth = $lw;
			$this->writer->write(sprintf('%.3F w', $lw * $this->scale_cons));
	...
	function SetLineWidth($width)
	{
		...
		$lwout = (sprintf('%.3F w', $width * $this->scale_cons));
	...
	{
		// Draw a line
		$this->writer->write(sprintf('%.3F %.3F m %.3F %.3F l S', $x1 * $this->scale_cons, ($this->h - $y1) * $this->scale_cons, $x2 * $this->scale_cons, ($this->h - $y2) * $this->scale_cons));
	}
	...
	function Arrow($x1, $y1, $x2, $y2, $headsize = 3, $fill = 'B', $angle = 25)
	{
		...
		$s = '';
		$s .= sprintf('%.3F %.3F m %.3F %.3F l S', $x1 * $this->scale_cons, ($this->h - $y1) * $this->scale_cons, $x5 * $this->scale_cons, $y5 * $this->scale_cons);
		$this->writer->write($s);

		$s = '';
		$s .= sprintf('%.3F %.3F m %.3F %.3F l %.3F %.3F l %.3F %.3F l %.3F %.3F l ', $x5 * $this->scale_cons, $y5 * $this->scale_cons, $x3 * $this->scale_cons, $y3 * $this->scale_cons, $x2 * $this->scale_cons, ($this->h - $y2) * $this->scale_cons, $x4 * $this->scale_cons, $y4 * $this->scale_cons, $x5 * $this->scale_cons, $y5 * $this->scale_cons);
		$s .= $fill;
		$this->writer->write($s);
	}
	...
	function Rect($x, $y, $w, $h, $style = '')
	{
		...
		$this->writer->write(sprintf('%.3F %.3F %.3F %.3F re %s', $x * $this->scale_cons, ($this->h - $y) * $this->scale_cons, $w * $this->scale_cons, -$h * $this->scale_cons, $op));
	}
	...
	function SetFont($family, $style = '', $size = 0, $write = true, $forcewrite = false)
	{
		...
		if (!$this->onlyCoreFonts && !$this->usingCoreFont) {
			...
			$this->FontSize = $size / $this->scale_cons;
	...
	{
		$l = [$x * $this->scale_cons, $this->hPt - $y * $this->scale_cons, $w * $this->scale_cons, $h * $this->scale_cons, $link];
	...
	function Text($x, $y, $txt, $OTLdata = [], $textvar = 0, $aixextra = '', $coordsys = '', $return = false)
	{
		...
		if ($this->falseBoldWeight && strpos($this->ReqFontStyle, "B") !== false && strpos($this->FontStyle, "B") === false) {
			$s .= '2 Tr 1 J 1 j ';
			$s .= sprintf('%.3F w ', ($this->FontSize / 130) * $this->scale_cons * $this->falseBoldWeight);
		...
		$py = $y;
		if ($coordsys != 'SVG') {
			$px = $x * $this->scale_cons;
			$py = ($this->h - $y) * $this->scale_cons;
		}
		...
		if (($textvar & TextVars::FD_UNDERLINE) && $txt != '') { // mPDF 5.7.1
			...
			$olw = $this->LineWidth;
			$s .= ' ' . (sprintf(' %.3F w', $ut * $this->scale_cons));
			$s .= ' ' . $this->_dounderline($x, $y + $adjusty, $txt, $OTLdata, $textvar);
			$s .= ' ' . (sprintf(' %.3F w', $olw * $this->scale_cons));
			if ($this->FillColor != $c) {
				$s.= ' ' . $this->FillColor . ' ';
			}
		}
	...
	function Cell($w, $h = 0, $txt = '', $border = 0, $ln = 0, $align = '', $fill = 0, $link = '', $currentx = 0, $lcpaddingL = 0, $lcpaddingR = 0, $valign = 'M', $spanfill = 0, $exactWidth = false, $OTLdata = false, $textvar = 0, $lineBox = false)
	{
		...
		if ($fill == 1 || $border == 1 || !empty($this->spanborddet)) {
			...
			if (!empty($this->spanborddet)) {
				...
				if ($fill == 1) {
					$s .= sprintf('%.3F %.3F %.3F %.3F re f ', ($this->x - $lbw) * $this->scale_cons, ($this->h - $bg_boxtop + $tbw) * $this->scale_cons, ($w + $lbw + $rbw) * $this->scale_cons, (-$bg_boxheight - $tbw - $bbw) * $this->scale_cons);
				}
				...
				if ($tbw) {
					...
					if ($this->spanborddet['T']['style'] == 'dashed') {
						$s .= sprintf(' 0 j 0 J [%.3F %.3F] 0 d ', $tbw * $dashon * $this->scale_cons, $tbw * $dashoff * $this->scale_cons);
					} elseif ($this->spanborddet['T']['style'] == 'dotted') {
						$s .= sprintf(' 1 j 1 J [%.3F %.3F] %.3F d ', 0.001, $tbw * $dot * $this->scale_cons, -$tbw / 2 * $this->scale_cons);
						$short = $tbw / 2;
					} else {
						$s .= ' 0 j 0 J [] 0 d ';
					}
					...
					if ($this->spanborddet['T']['style'] != 'dotted') {
						$s .= 'q ';
						$s .= sprintf('%.3F %.3F m ', ($this->x - $lbw) * $this->scale_cons, ($this->h - $bord_boxtop + $tbw) * $this->scale_cons);
						$s .= sprintf('%.3F %.3F l ', ($this->x + $w + $rbw) * $this->scale_cons, ($this->h - $bord_boxtop + $tbw) * $this->scale_cons);
						$s .= sprintf('%.3F %.3F l ', ($this->x + $w) * $this->scale_cons, ($this->h - $bord_boxtop) * $this->scale_cons);
						$s .= sprintf('%.3F %.3F l ', ($this->x) * $this->scale_cons, ($this->h - $bord_boxtop) * $this->scale_cons);
					...
					if ($this->spanborddet['T']['style'] == 'double') {
						$s .= sprintf(' %s %.3F w ', $c, $tbw / 3 * $this->scale_cons);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x - $lbw) * $this->scale_cons, ($this->h - $bord_boxtop + $tbw * 5 / 6) * $this->scale_cons, ($this->x + $w + $rbw) * $this->scale_cons, ($this->h - $bord_boxtop + $tbw * 5 / 6) * $this->scale_cons);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x - $lbw) * $this->scale_cons, ($this->h - $bord_boxtop + $tbw / 6) * $this->scale_cons, ($this->x + $w + $rbw) * $this->scale_cons, ($this->h - $bord_boxtop + $tbw / 6) * $this->scale_cons);
					} elseif ($this->spanborddet['T']['style'] == 'dotted') {
						$s .= sprintf(' %s %.3F w ', $c, $tbw * $this->scale_cons);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x - $lbw) * $this->scale_cons, ($this->h - $bord_boxtop + $tbw / 2) * $this->scale_cons, ($this->x + $w + $rbw - $short) * $this->scale_cons, ($this->h - $bord_boxtop + $tbw / 2) * $this->scale_cons);
					} else {
						$s .= sprintf(' %s %.3F w ', $c, $tbw * $this->scale_cons);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x - $lbw) * $this->scale_cons, ($this->h - $bord_boxtop + $tbw / 2) * $this->scale_cons, ($this->x + $w + $rbw - $short) * $this->scale_cons, ($this->h - $bord_boxtop + $tbw / 2) * $this->scale_cons);
					}
				...
				if ($bbw) {
					...
					if ($this->spanborddet['B']['style'] == 'dashed') {
						$s .= sprintf(' 0 j 0 J [%.3F %.3F] 0 d ', $bbw * $dashon * $this->scale_cons, $bbw * $dashoff * $this->scale_cons);
					} elseif ($this->spanborddet['B']['style'] == 'dotted') {
						$s .= sprintf(' 1 j 1 J [%.3F %.3F] %.3F d ', 0.001, $bbw * $dot * $this->scale_cons, -$bbw / 2 * $this->scale_cons);
						$short = $bbw / 2;
					} else {
						$s .= ' 0 j 0 J [] 0 d ';
					}
					...
					if ($this->spanborddet['B']['style'] != 'dotted') {
						$s .= 'q ';
						$s .= sprintf('%.3F %.3F m ', ($this->x - $lbw) * $this->scale_cons, ($this->h - $bord_boxbottom - $bbw) * $this->scale_cons);
						$s .= sprintf('%.3F %.3F l ', ($this->x + $w + $rbw) * $this->scale_cons, ($this->h - $bord_boxbottom - $bbw) * $this->scale_cons);
						$s .= sprintf('%.3F %.3F l ', ($this->x + $w) * $this->scale_cons, ($this->h - $bord_boxbottom) * $this->scale_cons);
						$s .= sprintf('%.3F %.3F l ', ($this->x) * $this->scale_cons, ($this->h - $bord_boxbottom) * $this->scale_cons);
						$s .= ' h W n '; // Ends path no-op & Sets the clipping path
					}
					...
					if ($this->spanborddet['B']['style'] == 'double') {
						$s .= sprintf(' %s %.3F w ', $c, $bbw / 3 * $this->scale_cons);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x - $lbw) * $this->scale_cons, ($this->h - $bord_boxbottom - $bbw / 6) * $this->scale_cons, ($this->x + $w + $rbw - $short) * $this->scale_cons, ($this->h - $bord_boxbottom - $bbw / 6) * $this->scale_cons);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x - $lbw) * $this->scale_cons, ($this->h - $bord_boxbottom - $bbw * 5 / 6) * $this->scale_cons, ($this->x + $w + $rbw - $short) * $this->scale_cons, ($this->h - $bord_boxbottom - $bbw * 5 / 6) * $this->scale_cons);
					} elseif ($this->spanborddet['B']['style'] == 'dotted') {
						$s .= sprintf(' %s %.3F w ', $c, $bbw * $this->scale_cons);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x - $lbw) * $this->scale_cons, ($this->h - $bord_boxbottom - $bbw / 2) * $this->scale_cons, ($this->x + $w + $rbw - $short) * $this->scale_cons, ($this->h - $bord_boxbottom - $bbw / 2) * $this->scale_cons);
					} else {
						$s .= sprintf(' %s %.3F w ', $c, $bbw * $this->scale_cons);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x - $lbw) * $this->scale_cons, ($this->h - $bord_boxbottom - $bbw / 2) * $this->scale_cons, ($this->x + $w + $rbw - $short) * $this->scale_cons, ($this->h - $bord_boxbottom - $bbw / 2) * $this->scale_cons);
					}
				...
				if ($lbw) {
					...
					if ($this->spanborddet['L']['style'] == 'dashed') {
						$s .= sprintf(' 0 j 0 J [%.3F %.3F] 0 d ', $lbw * $dashon * $this->scale_cons, $lbw * $dashoff * $this->scale_cons);
					} elseif ($this->spanborddet['L']['style'] == 'dotted') {
						$s .= sprintf(' 1 j 1 J [%.3F %.3F] %.3F d ', 0.001, $lbw * $dot * $this->scale_cons, -$lbw / 2 * $this->scale_cons);
						$short = $lbw / 2;
					} else {
						$s .= ' 0 j 0 J [] 0 d ';
					}
					...
					if ($this->spanborddet['L']['style'] != 'dotted') {
						...
						$s .= sprintf('%.3F %.3F l ', ($this->x) * $this->scale_cons, ($this->h - $bord_boxbottom) * $this->scale_cons);
						$s .= sprintf('%.3F %.3F l ', ($this->x) * $this->scale_cons, ($this->h - $bord_boxtop) * $this->scale_cons);
						$s .= sprintf('%.3F %.3F l ', ($this->x - $lbw) * $this->scale_cons, ($this->h - $bord_boxtop + $tbw) * $this->scale_cons);
						$s .= ' h W n '; // Ends path no-op & Sets the clipping path
					}

					$c = $this->SetDColor($this->spanborddet['L']['c'], true);
					if ($this->spanborddet['L']['style'] == 'double') {
						$s .= sprintf(' %s %.3F w ', $c, $lbw / 3 * $this->scale_cons);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x - $lbw / 6) * $this->scale_cons, ($this->h - $bord_boxtop + $tbw) * $this->scale_cons, ($this->x - $lbw / 6) * $this->scale_cons, ($this->h - $bord_boxbottom - $bbw + $short) * $this->scale_cons);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x - $lbw * 5 / 6) * $this->scale_cons, ($this->h - $bord_boxtop + $tbw) * $this->scale_cons, ($this->x - $lbw * 5 / 6) * $this->scale_cons, ($this->h - $bord_boxbottom - $bbw + $short) * $this->scale_cons);
					} elseif ($this->spanborddet['L']['style'] == 'dotted') {
						$s .= sprintf(' %s %.3F w ', $c, $lbw * $this->scale_cons);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x - $lbw / 2) * $this->scale_cons, ($this->h - $bord_boxtop + $tbw) * $this->scale_cons, ($this->x - $lbw / 2) * $this->scale_cons, ($this->h - $bord_boxbottom - $bbw + $short) * $this->scale_cons);
					} else {
						$s .= sprintf(' %s %.3F w ', $c, $lbw * $this->scale_cons);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x - $lbw / 2) * $this->scale_cons, ($this->h - $bord_boxtop + $tbw) * $this->scale_cons, ($this->x - $lbw / 2) * $this->scale_cons, ($this->h - $bord_boxbottom - $bbw + $short) * $this->scale_cons);
					}
				...
				if ($rbw) {
					...
					if ($this->spanborddet['R']['style'] == 'dashed') {
						$s .= sprintf(' 0 j 0 J [%.3F %.3F] 0 d ', $rbw * $dashon * $this->scale_cons, $rbw * $dashoff * $this->scale_cons);
					} elseif ($this->spanborddet['R']['style'] == 'dotted') {
						$s .= sprintf(' 1 j 1 J [%.3F %.3F] %.3F d ', 0.001, $rbw * $dot * $this->scale_cons, -$rbw / 2 * $this->scale_cons);
						$short = $rbw / 2;
					} else {
						$s .= ' 0 j 0 J [] 0 d ';
					}
					...
					if ($this->spanborddet['R']['style'] != 'dotted') {
						$s .= 'q ';
						$s .= sprintf('%.3F %.3F m ', ($this->x + $w + $rbw) * $this->scale_cons, ($this->h - $bord_boxbottom - $bbw) * $this->scale_cons);
						$s .= sprintf('%.3F %.3F l ', ($this->x + $w) * $this->scale_cons, ($this->h - $bord_boxbottom) * $this->scale_cons);
						$s .= sprintf('%.3F %.3F l ', ($this->x + $w) * $this->scale_cons, ($this->h - $bord_boxtop) * $this->scale_cons);
						$s .= sprintf('%.3F %.3F l ', ($this->x + $w + $rbw) * $this->scale_cons, ($this->h - $bord_boxtop + $tbw) * $this->scale_cons);
						$s .= ' h W n '; // Ends path no-op & Sets the clipping path
					...
					if ($this->spanborddet['R']['style'] == 'double') {
						$s .= sprintf(' %s %.3F w ', $c, $rbw / 3 * $this->scale_cons);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x + $w + $rbw / 6) * $this->scale_cons, ($this->h - $bord_boxtop + $tbw) * $this->scale_cons, ($this->x + $w + $rbw / 6) * $this->scale_cons, ($this->h - $bord_boxbottom - $bbw + $short) * $this->scale_cons);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x + $w + $rbw * 5 / 6) * $this->scale_cons, ($this->h - $bord_boxtop + $tbw) * $this->scale_cons, ($this->x + $w + $rbw * 5 / 6) * $this->scale_cons, ($this->h - $bord_boxbottom - $bbw + $short) * $this->scale_cons);
					} elseif ($this->spanborddet['R']['style'] == 'dotted') {
						$s .= sprintf(' %s %.3F w ', $c, $rbw * $this->scale_cons);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x + $w + $rbw / 2) * $this->scale_cons, ($this->h - $bord_boxtop + $tbw) * $this->scale_cons, ($this->x + $w + $rbw / 2) * $this->scale_cons, ($this->h - $bord_boxbottom - $bbw + $short) * $this->scale_cons);
					} else {
						$s .= sprintf(' %s %.3F w ', $c, $rbw * $this->scale_cons);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x + $w + $rbw / 2) * $this->scale_cons, ($this->h - $bord_boxtop + $tbw) * $this->scale_cons, ($this->x + $w + $rbw / 2) * $this->scale_cons, ($this->h - $bord_boxbottom - $bbw + $short) * $this->scale_cons);
					}
				...
				$s .= sprintf('%.3F %.3F %.3F %.3F re %s ', $this->x * $this->scale_cons, ($this->h - $bg_boxtop) * $this->scale_cons, $w * $this->scale_cons, -$bg_boxheight * $this->scale_cons, $op);
			}
		}
		...
		if (is_string($border)) { // If "border", does not come from WriteFlowingBlock or FinishFlowingBlock
			...
			if (is_int(strpos($border, 'L'))) {
				$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', $x * $this->scale_cons, ($this->h - $bord_boxtop) * $this->scale_cons, $x * $this->scale_cons, ($this->h - ($bord_boxbottom)) * $this->scale_cons);
			}

			if (is_int(strpos($border, 'T'))) {
				$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', $x * $this->scale_cons, ($this->h - $bord_boxtop) * $this->scale_cons, ($x + $w) * $this->scale_cons, ($this->h - $bord_boxtop) * $this->scale_cons);
			}

			if (is_int(strpos($border, 'R'))) {
				$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($x + $w) * $this->scale_cons, ($this->h - $bord_boxtop) * $this->scale_cons, ($x + $w) * $this->scale_cons, ($this->h - ($bord_boxbottom)) * $this->scale_cons);
			}

			if (is_int(strpos($border, 'B'))) {
				$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', $x * $this->scale_cons, ($this->h - ($bord_boxbottom)) * $this->scale_cons, ($x + $w) * $this->scale_cons, ($this->h - ($bord_boxbottom)) * $this->scale_cons);
			}
		}
		...
		if ($txt != '') {
			...
			if ($exactWidth) {
				...
				$stringWidth = $this->GetStringWidth($txt, true, $OTLdata, $textvar) + ( $this->charspacing * mb_strlen($txt, $this->mb_enc) / $this->scale_cons ) + ( $this->ws * mb_substr_count($txt, ' ', $this->mb_enc) / $this->scale_cons );
			}
			...
			if (isset($this->textparam['outline-s']) && $this->textparam['outline-s'] && !($textvar & TextVars::FC_SMALLCAPS)) { // mPDF 5.7.1
				$s .=' ' . sprintf('%.3F w', $this->LineWidth * $this->scale_cons) . ' ';
				$s .=" $this->DrawColor ";
				$s .=" 2 Tr ";
			...
			$px = ($this->x + $dx) * $this->scale_cons;
			$py = ($this->h - ($this->y + $glyphYorigin - $baseline_shift)) * $this->scale_cons;
			...
			if ($textvar & TextVars::FD_UNDERLINE) { // mPDF 5.7.1	// mPDF 6
				...
				$sub .= ' ' . (sprintf(' %.3F w 0 j 0 J ', $ut * $this->scale_cons));
				$sub .= ' ' . $this->_dounderline($this->x + $dx, $this->y + $ubaseline + $adjusty, $txt, $OTLdata, $textvar);
				$sub .= ' ' . (sprintf(' %.3F w 2 j 2 J ', $olw * $this->scale_cons));
			...
			if ($this->textshadow) {  // First to process is last in CSS comma separated shadows
				foreach ($this->textshadow as $ts) {
					...
					$s .= sprintf(' 1 0 0 1 %.4F %.4F cm', $ts['x'] * $this->scale_cons, -$ts['y'] * $this->scale_cons) . "\n";
					$s .= $sub;
					$s .= ' Q ';
				}
			}
	...
	function _kern($txt, $mode, $aix, $x, $y)
	{
		if ($mode == 'MBTw') { // Multibyte requiring word spacing
			...
			$space = $this->writer->utf8ToUtf16BigEndian($space, false);
			$space = $this->writer->escape($space);
			$s = sprintf(' BT ' . $aix, $x * $this->scale_cons, ($this->h - $y) * $this->scale_cons);
			$t = explode(' ', $txt);
			for ($i = 0; $i < count($t); $i++) {
				$tx = $t[$i];
			...
			$tj .= ')';
			$s .= sprintf(' BT ' . $aix . ' [%s] TJ ET ', $x * $this->scale_cons, ($this->h - $y) * $this->scale_cons, $tj);
		} else { // CORE Font
			$s = '';
			$tj = '(';
			$l = strlen($txt);
				...
				if ($l > $wmax) {
					...
					if ($sep == -1) { // Only one word
						...
						if ($align == 'J') {
							...
							list($charspacing, $ws, $kashida) = $this->GetJspacing($nb_carac, $nb_spaces, ((($wmax) - $len_ligne) * $this->scale_cons), $inclCursive, $tmpOTLdata);
							$this->SetSpacing($charspacing, $ws);
							//////////////////////////////////////////
						}
						if (isset($OTLdata)) {
				...
				if ($l > $wmax) {
					...
					if ($sep == -1) {
						...
						if ($align == 'J') {
							...
							$tmpOTLdata = [];
							list($charspacing, $ws, $kashida) = $this->GetJspacing($nb_carac, $nb_spaces, ((($wmax) - $len_ligne) * $this->scale_cons), false, $tmpOTLdata);
							$this->SetSpacing($charspacing, $ws);
							//////////////////////////////////////////
						}
	...
	function newFlowingBlock($w, $h, $a = '', $is_table = false, $blockstate = 0, $newblock = true, $blockdir = 'ltr', $table_draft = false)
	{
		...
		$this->flowingBlockAttr['width'] = ($w * $this->scale_cons);
	...
	function finishFlowingBlock($endofblock = false, $next = '')
	{
		...
		if ($is_table) {
			...
			$paddingL = ($ipaddingL * $this->scale_cons);
			$paddingR = ($ipaddingR * $this->scale_cons);
		...
		foreach ($content as $k => $chunk) {
			...
			if (!isset($this->objectbuffer[$k]) || (isset($this->objectbuffer[$k]) && !$this->objectbuffer[$k])) {
				...
				$contentWidth += $this->GetStringWidth($chunk, true, (isset($cOTLdata[$k]) ? $cOTLdata[$k] : false), $this->textvar) * $this->scale_cons;
				...
				if ($this->objectbuffer[$k]['type'] == 'image' && isset($this->objectbuffer[$k]['listmarker']) && $this->objectbuffer[$k]['listmarker'] && $this->objectbuffer[$k]['listmarkerposition'] == 'outside') {
					...
					$contentWidth += $this->objectbuffer[$k]['OUTER-WIDTH'] * $this->scale_cons;
				}
			}
		}
		...
		if ($blockdir == 'ltr' && strpos($lastfontreqstyle, "I") !== false && strpos($lastfontstyle, "I") === false) { // Artificial italic
			$lastitalic = $this->FontSize * 0.15 * $this->scale_cons;
		} else {
			$lastitalic = 0;
		}
		...
		if (($newblock) && ($blockstate == 1 || $blockstate == 3) && isset($this->blk[$this->blklvl]['text_indent']) && ($lineCount == 0) && (!$is_table) && ($align != 'C')) {
			...
			$WidthCorrection = ($ti * $this->scale_cons);
		}
		...
		if ($content) {
				...
				if (count($cOTLdata)) {
					...
					if ($aord == count($chunkorder) - 1 && isset($cOTLdata[$aord]['group'])) { // Last chunk on line
						...
						if (isset($cOTLdata[$aord]['GPOSinfo'][$nGPOS]['XAdvanceL']) || isset($cOTLdata[$aord]['GPOSinfo'][$nGPOS]['XAdvanceR'])) {
							...
							$w *= ($this->FontSize / 1000);
							$contentWidth -= $w * $this->scale_cons;
			...
			if ((((($contentWidth + $lastitalic) > $maxWidth) && ($content[(count($chunkorder) - 1)] != ' ') ) ||
				...
				// WORD SPACING
				list($jcharspacing, $jws, $jkashida) = $this->GetJspacing($nb_carac, $nb_spaces, ($maxWidth - $lastitalic - $contentWidth - $WidthCorrection - (($this->cMarginL + $this->cMarginR) * $this->scale_cons) - ($paddingL + $paddingR + (($fpaddingL + $fpaddingR) * $this->scale_cons) )), $inclCursive, $cOTLdata);
				...
				if (preg_match("/[" . $this->CJKoverflow . "]/u", $hanger)) {
					...
					$contentWidth -= $this->GetStringWidth($hanger) * $this->scale_cons;
					$nb_carac -= 1;
					list($jcharspacing, $jws, $jkashida) = $this->GetJspacing($nb_carac, $nb_spaces, ($maxWidth - $lastitalic - $contentWidth - $WidthCorrection - (($this->cMarginL + $this->cMarginR) * $this->scale_cons) - ($paddingL + $paddingR + (($fpaddingL + $fpaddingR) * $this->scale_cons) )), $inclCursive, $cOTLdata);
				}
			} /* -- END CJK-FONTS -- */
			...
			elseif ($contentWidth < ($maxWidth - $lastitalic - $WidthCorrection - (($this->cMarginL + $this->cMarginR) * $this->scale_cons) - ($paddingL + $paddingR + (($fpaddingL + $fpaddingR) * $this->scale_cons))) && !$this->fixedlSpacing) {
				...
				$check = $maxWidth - $lastitalic - $WidthCorrection - $contentWidth - (($this->cMarginL + $this->cMarginR) * $this->scale_cons) - ($paddingL + $paddingR + (($fpaddingL + $fpaddingR) * $this->scale_cons) ) - ( $jcharspacing * $nb_carac) - ( $jws * $nb_spaces);
				if ($check <= 0) {
					$jcharspacing = 0;
					$jws = 0;
				}
			...
			$empty = $maxWidth - $lastitalic - $WidthCorrection - $contentWidth - (($this->cMarginL + $this->cMarginR) * $this->scale_cons) - ($paddingL + $paddingR + (($fpaddingL + $fpaddingR) * $this->scale_cons) );
			...
			foreach ($chunkorder as $aord => $k) { // mPDF 5.7
				...
				$this->SetSpacing(($this->fixedlSpacing * $this->scale_cons) + $jcharspacing, ($this->fixedlSpacing + $this->minwSpacing) * $this->scale_cons + $jws);
				$this->fixedlSpacing = false;
				$this->minwSpacing = 0;
				...
				$stringWidth += ( $this->charspacing * $nch / $this->scale_cons );

				$stringWidth += ( $this->ws * mb_substr_count($chunk, ' ', $this->mb_enc) / $this->scale_cons );
	...
	function printobjectbuffer($is_table = false, $blockdir = false)
	{
		...
		foreach ($this->objectbuffer as $ib => $objattr) {
			...
			if ($objattr['type'] == 'image') {
				...
				$sx = $objattr['orig_w'] ? ($objattr['INNER-WIDTH'] * $this->scale_cons / $objattr['orig_w']) : INF;
				$sy = $objattr['orig_h'] ? ($objattr['INNER-HEIGHT'] * $this->scale_cons / $objattr['orig_h']) : INF;
				...
				if ($rotate == 90) {
					...
					$sx = $obiw * $this->scale_cons / $objattr['orig_h'];
					$sy = $obih * $this->scale_cons / $objattr['orig_w'];
				...
				if (isset($objattr['itype']) && $objattr['itype'] == 'wmf') {
					$outstring = sprintf('q ' . $tr . $tr2 . '%.3F 0 0 %.3F %.3F %.3F cm /FO%d Do Q', $sx, -$sy, $objattr['INNER-X'] * $this->scale_cons - $sx * $objattr['wmf_x'], (($this->h - $objattr['INNER-Y']) * $this->scale_cons) + $sy * $objattr['wmf_y'], $objattr['ID']); // mPDF 5.7.3 TRANSFORMS
				} else { 				/* -- END IMAGES-WMF -- */
					if (isset($objattr['itype']) && $objattr['itype'] == 'svg') {
						$outstring = sprintf('q ' . $tr . $tr2 . '%.3F 0 0 %.3F %.3F %.3F cm /FO%d Do Q', $sx, -$sy, $objattr['INNER-X'] * $this->scale_cons - $sx * $objattr['wmf_x'], (($this->h - $objattr['INNER-Y']) * $this->scale_cons) + $sy * $objattr['wmf_y'], $objattr['ID']); // mPDF 5.7.3 TRANSFORMS
					} else {
						$outstring = sprintf("q " . $tr . $tr2 . "%.3F 0 0 %.3F %.3F %.3F cm " . $gradmask . "/I%d Do Q", $obiw * $this->scale_cons, $obih * $this->scale_cons, $objattr['INNER-X'] * $this->scale_cons, ($this->h - ($objattr['INNER-Y'] + $obih )) * $this->scale_cons, $objattr['ID']); // mPDF 5.7.3 TRANSFORMS
					}
				}
	...
	function _printListBullet($x, $y, $size, $type, $color)
	{
		...
		if ($type == 'square') {
			...
			$this->writer->write(sprintf('%.3F %.3F %.3F %.3F re f', ($x - $size / 2) * $this->scale_cons, ($this->h - $y + $size / 2) * $this->scale_cons, ($size) * $this->scale_cons, (-$size) * $this->scale_cons));
		} elseif ($type == 'disc') {
	...
	// mPDF 5.7.1
		...
		if (substr($s, 0, 3) == "\xbb\xa4\xac") { // identifier has been identified!
			...
			$h_corr = 0;
			if ($is_table) { // *TABLES*
				$maximumW = ($maxWidth / $this->scale_cons) - ($this->cellPaddingL + $this->cMarginL + $this->cellPaddingR + $this->cMarginR);  // *TABLES*
			} // *TABLES*
			else { // *TABLES*
				if (($newblock) && ($blockstate == 1 || $blockstate == 3) && ($lineCount == 0) && (!$is_table)) {
					$h_corr = $this->blk[$this->blklvl]['padding_top'] + $this->blk[$this->blklvl]['border_top']['w'];
				}
				$maximumW = ($maxWidth / $this->scale_cons) - ($this->blk[$this->blklvl]['padding_left'] + $this->blk[$this->blklvl]['border_left']['w'] + $this->blk[$this->blklvl]['padding_right'] + $this->blk[$this->blklvl]['border_right']['w'] + $fpaddingL + $fpaddingR );
			} // *TABLES*
			$objattr = $this->inlineObject($objattr['type'], $this->lMargin + $fpaddingL + ($contentWidth / $this->scale_cons), ($this->y + $h_corr), $objattr, $this->lMargin, ($contentWidth / $this->scale_cons), $maximumW, $stackHeight, true, $is_table);
			...
			if ($objattr['type'] == 'image' && isset($objattr['listmarker']) && $objattr['listmarker'] && $objattr['listmarkerposition'] == 'outside') {
				...
				$contentWidth += ($objattr['OUTER-WIDTH'] * $this->scale_cons);
			}
			return;
		}
		...
		for ($i = 0; $i < $clen; $i++) {
			...
			if ($this->usingCoreFont) {
				$c = $s[$i];
				// Soft Hyphens chr(173)
				$cw = ($this->GetCharWidthCore($c) * $this->scale_cons);
				...
				$c = mb_substr($s, $i, 1, $this->mb_enc);
				$cw = ($this->GetCharWidthNonCore($c, false) * $this->scale_cons);
				// mPDF 5.7.1
				// Use OTL GPOS
				if (isset($this->CurrentFont['useOTL']) && ($this->CurrentFont['useOTL'] & 0xFF)) {
					...
					if (isset($sOTLdata['GPOSinfo'][$i]['XAdvanceL'])) {
						$cw += $sOTLdata['GPOSinfo'][$i]['XAdvanceL'] * (1000 / $this->CurrentFont['unitsPerEm']) * ($this->FontSize / 1000) * $this->scale_cons;
					}
				}
				if (($this->textvar & TextVars::FC_KERNING) && $i > 0) { // mPDF 5.7.1
			...
			if ($i == 0) {
				$cw += $lbw * $this->scale_cons;
				$contentB[(count($contentB) - 1)] .= 'L';
			}
			if ($i == ($clen - 1)) {
				$cw += $rbw * $this->scale_cons;
			...
			foreach ($this->objectbuffer as $k => $objattr) {   // mPDF 6 DOTTAB
				if ($objattr['type'] == 'dottab') {
					$WidthCorrection -= ($objattr['outdent'] * $this->scale_cons);
					break;
				}
			}
			...
			if (( $contentWidth + $cw > $maxWidth - $WidthCorrection - (($this->cMarginL + $this->cMarginR) * $this->scale_cons) - ($paddingL + $paddingR + (($fpaddingL + $fpaddingR) * $this->scale_cons) ) + 0.001)) {// 0.001 is to correct for deviations converting mm=>pts
				...
				foreach ($content as $k => $chunk) {
					if (isset($this->objectbuffer[$k]) && $this->objectbuffer[$k]) {
						...
						$contentWidth += $this->GetStringWidth($chunk, true, (isset($cOTLdata[$k]) ? $cOTLdata[$k] : false), $this->textvar) * $this->scale_cons;  // mPDF 5.7.1
						if (!empty($this->spanborddet)) {
							if (isset($this->spanborddet['L']['w']) && strpos($contentB[$k], 'L') !== false) {
								$contentWidth += $this->spanborddet['L']['w'] * $this->scale_cons;
							}
							if (isset($this->spanborddet['R']['w']) && strpos($contentB[$k], 'R') !== false) {
								$contentWidth += $this->spanborddet['R']['w'] * $this->scale_cons;
							}
						}
					}
				}
				...
				if (!$table_draft) {
					...
					if (($align == 'J' && !$CJKoverflow) || (($contentWidth + $lastitalic > $maxWidth - $WidthCorrection - (($this->cMarginL + $this->cMarginR) * $this->scale_cons) - ($paddingL + $paddingR + (($fpaddingL + $fpaddingR) * $this->scale_cons) ) + 0.001) && (!$CJKoverflow || ($CJKoverflow && !$this->allowCJKoverflow))) || $CJKoverflow && $align == 'J' && $this->allowCJKoverflow && $hanger && $this->CJKforceend) {   // 0.001 is to correct for deviations converting mm=>pts
					...
					foreach ($chunkorder as $aord => $k) { // mPDF 5.7
						...
						if ($aord == $arraysize - 1) {
							$stringWidth -= ( $this->charspacing / $this->scale_cons );
							if ($this->checkCJK && $CJKoverflow && $align == 'J' && $this->allowCJKoverflow && $hanger && $this->CJKforceend) {
								// force-end overhang
				...
				if (!empty($savedObj)) {
					...
					$contentB[] = '';
					$cOTLdata[] = []; // mPDF 5.7.1
					$contentWidth += $savedObj['OUTER-WIDTH'] * $this->scale_cons;
				}
				if (count($savedPreContent) > 0) {
					for ($ix = count($savedPreContent) - 1; $ix >= 0; $ix--) {
						...
						if ($ix > 0) {
							$contentWidth += $this->GetStringWidth($savedPreContent[$ix], true, (isset($savedPreOTLdata[$ix]) ? $savedPreOTLdata[$ix] : false), $this->textvar) * $this->scale_cons; // mPDF 5.7.1
				...
				$contentWidth += $this->GetStringWidth($currContent, false, (isset($cOTLdata[(count($cOTLdata) - 1)]) ? $cOTLdata[(count($cOTLdata) - 1)] : false), $this->textvar) * $this->scale_cons; // mPDF 5.7.1
	...
	function Image($file, $x, $y, $w = 0, $h = 0, $type = '', $link = '', $paint = true, $constrain = true, $watermark = false, $shownoimg = true, $allowvector = true)
	{
		...
		if ($w == 0 and $h == 0) {
			...
			if ($info['type'] == 'wmf') {
				...
				$w = abs($info['w']) / (20 * $this->scale_cons);
				$h = abs($info['h']) / (20 * $this->scale_cons);
			} else { 			/* -- END IMAGES-WMF -- */
				if ($info['type'] == 'svg') {
					// returned SVG units are pts
					// divide by k to get user units (mm)
					$w = abs($info['w']) / $this->scale_cons;
					$h = abs($info['h']) / $this->scale_cons;
				} else {
					// Put image at default image dpi
					$w = ($info['w'] / $this->scale_cons) * (72 / $this->img_dpi);
					$h = ($info['h'] / $this->scale_cons) * (72 / $this->img_dpi);
				}
			}
		}
		if ($w == 0) {
		...
		if ($watermark) {
			...
			/* -- IMAGES-WMF -- */
			if ($info['type'] == 'wmf') {
				$sx = $w * $this->scale_cons / $info['w'];
				$sy = -$h * $this->scale_cons / $info['h'];
				$outstring = sprintf('q %.3F 0 0 %.3F %.3F %.3F cm /FO%d Do Q', $sx, $sy, $x * $this->scale_cons - $sx * $info['x'], (($this->h - $y) * $this->scale_cons) - $sy * $info['y'], $info['i']);
			} else { 			/* -- END IMAGES-WMF -- */
				if ($info['type'] == 'svg') {
					$sx = $w * $this->scale_cons / $info['w'];
					$sy = -$h * $this->scale_cons / $info['h'];
					...
					$outstring = sprintf("q %.3F 0 0 %.3F %.3F %.3F cm /I%d Do Q", $w * $this->scale_cons, $h * $this->scale_cons, $x * $this->scale_cons, ($this->h - ($y + $h)) * $this->scale_cons, $info['i']);
				}
			}
	...
	{
		if ($black and $white) {
			$s = sprintf('[%.3F %.3F] 0 d', $black * $this->scale_cons, $white * $this->scale_cons);
		} else {
			$s = '[] 0 d';
		}
	...
	function _puthtmlheaders()
	{
		...
		for ($n = 1; $n <= $nb; $n++) {
			...
			if (isset($this->saveHTMLHeader[$n][$OE])) {
				...
				$os = '';
				if ($rotate) {
					$os .= sprintf('q 0 -1 1 0 0 %.3F cm ', ($this->w * $this->scale_cons));
					// To rotate the other way i.e. Header to left of page:
					// $os .= sprintf('q 0 1 -1 0 %.3F %.3F cm ',($this->h*$this->scale_cons), (($this->rMargin - $this->lMargin )*$this->scale_cons));
				}
				...
				foreach ($lks as $lk) {
					if ($rotate) {
						...
						$ax = $lk[0] / $this->scale_cons;
						$ay = $lk[1] / $this->scale_cons;
						$bx = $ay - ($lh / $this->scale_cons);
						$by = $this->w - $ax;
						$lk[0] = $bx * $this->scale_cons;
						$lk[1] = ($this->h - $by) * $this->scale_cons - $lw;
					}
					$this->PageLinks[$n][] = $lk;
				}
				/* -- FORMS -- */
	...
	function _dounderline($x, $y, $txt, $OTLdata = false, $textvar = 0)
	{
		...
		// WORD SPACING
		$w = ($this->GetStringWidth($txt, false, $OTLdata, $textvar) * $this->scale_cons) + ($this->charspacing * mb_strlen($txt, $this->mb_enc)) + ( $this->ws * mb_substr_count($txt, ' ', $this->mb_enc));
		// Draw a line
		return sprintf('%.3F %.3F m %.3F %.3F l S', $x * $this->scale_cons, ($this->h - $y) * $this->scale_cons, ($x * $this->scale_cons) + $w, ($this->h - $y) * $this->scale_cons);
	}
	...
	function watermark($texte, $angle = 45, $fontsize = 96, $alpha = 0.2)
	{
		...
		while ($loop == 0) {
			...
			$offset = ((sin(deg2rad($angle))) * ($szfont / $this->scale_cons));
	...
	function Rotate($angle, $x = -1, $y = -1)
	{
		...
		if ($angle != 0) {
			...
			$c = cos($angle);
			$s = sin($angle);
			$cx = $x * $this->scale_cons;
			$cy = ($this->h - $y) * $this->scale_cons;
	...
	function RoundedRect($x, $y, $w, $h, $r, $style = '')
	{
		...
		$MyArc = 4 / 3 * (sqrt(2) - 1);
		$this->writer->write(sprintf('%.3F %.3F m', ($x + $r) * $this->scale_cons, ($hp - $y) * $this->scale_cons));
		$xc = $x + $w - $r;
		$yc = $y + $r;
		$this->writer->write(sprintf('%.3F %.3F l', $xc * $this->scale_cons, ($hp - $y) * $this->scale_cons));

		$this->_Arc($xc + $r * $MyArc, $yc - $r, $xc + $r, $yc - $r * $MyArc, $xc + $r, $yc);
		$xc = $x + $w - $r;
		$yc = $y + $h - $r;
		$this->writer->write(sprintf('%.3F %.3F l', ($x + $w) * $this->scale_cons, ($hp - $yc) * $this->scale_cons));

		$this->_Arc($xc + $r, $yc + $r * $MyArc, $xc + $r * $MyArc, $yc + $r, $xc, $yc + $r);
		$xc = $x + $r;
		$yc = $y + $h - $r;
		$this->writer->write(sprintf('%.3F %.3F l', $xc * $this->scale_cons, ($hp - ($y + $h)) * $this->scale_cons));

		$this->_Arc($xc - $r * $MyArc, $yc + $r, $xc - $r, $yc + $r * $MyArc, $xc - $r, $yc);
		$xc = $x + $r;
		$yc = $y + $r;
		$this->writer->write(sprintf('%.3F %.3F l', ($x) * $this->scale_cons, ($hp - $yc) * $this->scale_cons));
	...
	{
		$h = $this->h;
		$this->writer->write(sprintf('%.3F %.3F %.3F %.3F %.3F %.3F c ', $x1 * $this->scale_cons, ($h - $y1) * $this->scale_cons, $x2 * $this->scale_cons, ($h - $y2) * $this->scale_cons, $x3 * $this->scale_cons, ($h - $y3) * $this->scale_cons));
	}
		...
		foreach ($content as $k => $chunk) {
			if (isset($this->objectbuffer[$k])) {
				...
				if ($this->objectbuffer[$k]['type'] == 'listmarker') {
					...
					$oh = $font[$k]['size']; // == $this->objectbuffer[$k]['fontsize']/$this->scale_cons;
				...
				foreach ($content[$i] as $tablehf) {
					...
					if (isset($tablehf['background-image']) && $paintcell) {
						if ($tablehf['background-image']['gradient'] && preg_match('/(-moz-)*(repeating-)*(linear|radial)-gradient/', $tablehf['background-image']['gradient'])) {
							...
							if ($this->ColActive) {
								...
								$this->writer->write(sprintf('q /Pattern cs /P%d scn %s %.3F %.3F %.3F %.3F re f Q', $n, $opac, $px * $this->scale_cons, ($this->h - $py) * $this->scale_cons, $pw * $this->scale_cons, -$ph * $this->scale_cons));
							} else {
					...
					if (!empty($textbuffer)) {
						...
						if ($R) {
							...
							$cellFontHeight = ($cellPtSize / $this->scale_cons);
							$opx = $this->x;
							$opy = $this->y;
							$angle = intval($R);
	...
	function WriteFixedPosHTML($html, $x, $y, $w, $h, $overflow = 'visible', $bounding = [])
	{
		...
		if ($overflow == 'hidden') {
			...
			$this->writer->write("q");
			$ch = $clip_y1 - $y;
			$this->writer->write(sprintf('%.3F %.3F %.3F %.3F re %s', $x * $this->scale_cons, ($this->h - $y) * $this->scale_cons, $w * $this->scale_cons, -$ch * $this->scale_cons, $op));
			if (!empty($block_s)) {
				$tmp = "q\n" . sprintf('%.3F %.3F %.3F %.3F re %s', $x * $this->scale_cons, ($this->h - $y) * $this->scale_cons, $w * $this->scale_cons, -$ch * $this->scale_cons, $op);
				$tmp .= "\n" . $block_s . "\nQ";
				$block_s = $tmp;
			}
		}
		...
		foreach ($this->HTMLheaderPageLinks as $lk) {
			if ($rotate) {
				...
				$lx1 = (($lk[0] / $this->scale_cons));
				$ly1 = (($this->h - ($lk[1] / $this->scale_cons)));
				if ($rotate == 90) {
				...
				$lk[0] = $lx1 * $this->scale_cons;
				$lk[1] = ($this->h - $ly1) * $this->scale_cons;
			}
			if ($shrink_f != 1) {  // i.e. autofit has resized the box
				$lx1 = (($lk[0] / $this->scale_cons) - $x);
				$lx2 = $x + ($lx1 * $shrink_f);
				$lk[0] = $lx2 * $this->scale_cons;
				$ly1 = (($this->h - ($lk[1] / $this->scale_cons)) - $y);
				$ly2 = $y + ($ly1 * $shrink_f);
				$lk[1] = ($this->h - $ly2) * $this->scale_cons;
				$lk[2] *= $shrink_f; // width
		...
		for ($i = 0; $i < $array_size; $i++) {
			...
			if (substr($vetor[0], 0, 3) == "\xbb\xa4\xac") { // identifier has been identified!
				...
				if ($objattr['type'] == 'nestedtable') {
					...
					list($skipln) = $this->inlineObject($objattr['type'], '', $this->y, $objattr, $this->lMargin, ($this->flowingBlockAttr['contentWidth'] / $this->scale_cons), $maxWidth, $this->flowingBlockAttr['height'], false, $is_table);
					...
					if ($objattr['type'] == 'image' && isset($objattr['float'])) {
						...
						$widthLeft = $maxWidth - ($this->flowingBlockAttr['contentWidth'] / $this->scale_cons);
	...
	function PaintDivBB($divider = '', $blockstate = 0, $blvl = 0)
	{
		...
		if ($this->blk[$blvl]['border_top'] && $divider != 'pagetop' && !$continuingpage) {
			...
			if (isset($tbd['s']) && $tbd['s']) {
				if (!$brset && $tbd['style'] != 'dotted' && $tbd['style'] != 'dashed') {
					$this->writer->write('q');
					$this->SetLineWidth(0);
					$this->writer->write(sprintf('%.3F %.3F m ', ($x0) * $this->scale_cons, ($this->h - ($y0)) * $this->scale_cons));
					$this->writer->write(sprintf('%.3F %.3F l ', ($x0 + $border_left) * $this->scale_cons, ($this->h - ($y0 + $border_top)) * $this->scale_cons));
					$this->writer->write(sprintf('%.3F %.3F l ', ($x0 + $w - $border_right) * $this->scale_cons, ($this->h - ($y0 + $border_top)) * $this->scale_cons));
					$this->writer->write(sprintf('%.3F %.3F l ', ($x0 + $w) * $this->scale_cons, ($this->h - ($y0)) * $this->scale_cons));
				...
				if ($brTR_H && $brTR_V) {
					...
					if ($tbd['style'] == 'solid' || $tbd['style'] == 'double') {
						$s .= (sprintf('%.3F %.3F m ', ($x0 + $w) * $this->scale_cons, ($this->h - ($y0 + ($border_top / 2))) * $this->scale_cons)) . "\n";
					} else {
						$s .= (sprintf('%.3F %.3F m ', ($x0 + $w - ($border_top / 2)) * $this->scale_cons, ($this->h - ($y0 + ($border_top / 2))) * $this->scale_cons)) . "\n";
					}
				}
				/* -- BORDER-RADIUS -- */
				if ($brTL_H && $brTL_V) {
					if ($legend) {
						if ($legbreakR < ($x0 + $w - $brTR_H)) {
							$s .= (sprintf('%.3F %.3F l ', $legbreakR * $this->scale_cons, ($this->h - ($y0 + ($border_top / 2))) * $this->scale_cons)) . "\n";
						}
						if ($legbreakL > ($x0 + $brTL_H )) {
							$s .= (sprintf('%.3F %.3F m ', $legbreakL * $this->scale_cons, ($this->h - ($y0 + ($border_top / 2))) * $this->scale_cons)) . "\n";
							$s .= (sprintf('%.3F %.3F l ', ($x0 + $brTL_H ) * $this->scale_cons, ($this->h - ($y0 + ($border_top / 2))) * $this->scale_cons) . "\n");
						} else {
							$s .= (sprintf('%.3F %.3F m ', ($x0 + $brTL_H ) * $this->scale_cons, ($this->h - ($y0 + ($border_top / 2))) * $this->scale_cons)) . "\n";
						}
					} else {
						$s .= (sprintf('%.3F %.3F l ', ($x0 + $brTL_H ) * $this->scale_cons, ($this->h - ($y0 + ($border_top / 2))) * $this->scale_cons)) . "\n";
					}
					...
					if ($legend) {
						...
						if ($legbreakL > ($x0)) {
							$s .= (sprintf('%.3F %.3F m ', $legbreakL * $this->scale_cons, ($this->h - ($y0 + ($border_top / 2))) * $this->scale_cons)) . "\n";
							if ($tbd['style'] == 'solid' || $tbd['style'] == 'double') {
								$s .= (sprintf('%.3F %.3F l ', ($x0) * $this->scale_cons, ($this->h - ($y0 + ($border_top / 2))) * $this->scale_cons)) . "\n";
							} else {
								$s .= (sprintf('%.3F %.3F l ', ($x0 + ($border_top / 2)) * $this->scale_cons, ($this->h - ($y0 + ($border_top / 2))) * $this->scale_cons)) . "\n";
							}
						} elseif ($tbd['style'] == 'solid' || $tbd['style'] == 'double') {
							$s .= (sprintf('%.3F %.3F m ', ($x0) * $this->scale_cons, ($this->h - ($y0 + ($border_top / 2))) * $this->scale_cons)) . "\n";
						} else {
							$s .= (sprintf('%.3F %.3F m ', ($x0 + $border_top / 2) * $this->scale_cons, ($this->h - ($y0 + ($border_top / 2))) * $this->scale_cons)) . "\n";
						}
					} elseif ($tbd['style'] == 'solid' || $tbd['style'] == 'double') {
						$s .= (sprintf('%.3F %.3F l ', ($x0) * $this->scale_cons, ($this->h - ($y0 + ($border_top / 2))) * $this->scale_cons)) . "\n";
					} else {
		...
		if ($this->blk[$blvl]['border_bottom'] && $blockstate != 1 && $divider != 'pagebottom') {
			...
			if (isset($tbd['s']) && $tbd['s']) {
				if (!$brset && $tbd['style'] != 'dotted' && $tbd['style'] != 'dashed') {
					$this->writer->write('q');
					$this->SetLineWidth(0);
					$this->writer->write(sprintf('%.3F %.3F m ', ($x0) * $this->scale_cons, ($this->h - ($y0 + $h)) * $this->scale_cons));
					$this->writer->write(sprintf('%.3F %.3F l ', ($x0 + $border_left) * $this->scale_cons, ($this->h - ($y0 + $h - $border_bottom)) * $this->scale_cons));
					$this->writer->write(sprintf('%.3F %.3F l ', ($x0 + $w - $border_right) * $this->scale_cons, ($this->h - ($y0 + $h - $border_bottom)) * $this->scale_cons));
					$this->writer->write(sprintf('%.3F %.3F l ', ($x0 + $w) * $this->scale_cons, ($this->h - ($y0 + $h)) * $this->scale_cons));
					$this->writer->write(' h W n '); // Ends path no-op & Sets the clipping path
				}
				...
				if ($brBL_H && $brBL_V) {
					...
					if ($tbd['style'] == 'solid' || $tbd['style'] == 'double') {
						$s .= (sprintf('%.3F %.3F m ', ($x0) * $this->scale_cons, ($this->h - ($y0 + $h - ($border_bottom / 2))) * $this->scale_cons)) . "\n";
					} else {
						$s .= (sprintf('%.3F %.3F m ', ($x0 + ($border_bottom / 2)) * $this->scale_cons, ($this->h - ($y0 + $h - ($border_bottom / 2))) * $this->scale_cons)) . "\n";
					}
				}
				/* -- BORDER-RADIUS -- */
				if ($brBR_H && $brBR_V) {
					$s .= (sprintf('%.3F %.3F l ', ($x0 + $w - ($border_bottom / 2) - $brBR_H ) * $this->scale_cons, ($this->h - ($y0 + $h - ($border_bottom / 2))) * $this->scale_cons)) . "\n";
					$s .= ($this->_EllipseArc($x0 + $w - $brBR_H, $y0 + $h - $brBR_V, $brBR_H - $border_bottom / 2, $brBR_V - $border_bottom / 2, 4, 1)) . "\n";
				} else { 				/* -- END BORDER-RADIUS -- */
					if ($tbd['style'] == 'solid' || $tbd['style'] == 'double') {
						$s .= (sprintf('%.3F %.3F l ', ($x0 + $w) * $this->scale_cons, ($this->h - ($y0 + $h - ($border_bottom / 2))) * $this->scale_cons)) . "\n";
					} else {
						$s .= (sprintf('%.3F %.3F l ', ($x0 + $w - ($border_bottom / 2)) * $this->scale_cons, ($this->h - ($y0 + $h - ($border_bottom / 2))) * $this->scale_cons)) . "\n";
					}
				}
				$s .= 'S' . "\n";
				$this->writer->write($s);
		...
		if ($this->blk[$blvl]['border_left']) {
			...
			if (isset($tbd['s']) && $tbd['s']) {
				if (!$brset && $tbd['style'] != 'dotted' && $tbd['style'] != 'dashed') {
					...
					$this->writer->write(sprintf('%.3F %.3F l ', ($x0 + $border_left) * $this->scale_cons, ($this->h - ($y0 + $border_top)) * $this->scale_cons));
					$this->writer->write(sprintf('%.3F %.3F l ', ($x0 + $border_left) * $this->scale_cons, ($this->h - ($y0 + $h - $border_bottom)) * $this->scale_cons));
					$this->writer->write(sprintf('%.3F %.3F l ', ($x0) * $this->scale_cons, ($this->h - ($y0 + $h)) * $this->scale_cons));
					$this->writer->write(' h W n '); // Ends path no-op & Sets the clipping path
				}
				...
				if ($brTL_V && $brTL_H) {
					...
					if ($tbd['style'] == 'solid' || $tbd['style'] == 'double') {
						$s .= (sprintf('%.3F %.3F m ', ($x0 + ($border_left / 2)) * $this->scale_cons, ($this->h - ($y0)) * $this->scale_cons)) . "\n";
					} else {
						$s .= (sprintf('%.3F %.3F m ', ($x0 + ($border_left / 2)) * $this->scale_cons, ($this->h - ($y0 + ($border_left / 2))) * $this->scale_cons)) . "\n";
					}
				}
				/* -- BORDER-RADIUS -- */
				if ($brBL_V && $brBL_H) {
					$s .= (sprintf('%.3F %.3F l ', ($x0 + ($border_left / 2)) * $this->scale_cons, ($this->h - ($y0 + $h - ($border_left / 2) - $brBL_V) ) * $this->scale_cons)) . "\n";
					$s .= ($this->_EllipseArc($x0 + $brBL_H, $y0 + $h - $brBL_V, $brBL_H - $border_left / 2, $brBL_V - $border_left / 2, 3, 1)) . "\n";
				} else { 				/* -- END BORDER-RADIUS -- */
					if ($tbd['style'] == 'solid' || $tbd['style'] == 'double') {
						$s .= (sprintf('%.3F %.3F l ', ($x0 + ($border_left / 2)) * $this->scale_cons, ($this->h - ($y0 + $h) ) * $this->scale_cons)) . "\n";
					} else {
						$s .= (sprintf('%.3F %.3F l ', ($x0 + ($border_left / 2)) * $this->scale_cons, ($this->h - ($y0 + $h - ($border_left / 2)) ) * $this->scale_cons)) . "\n";
					}
				}
				$s .= 'S' . "\n";
				$this->writer->write($s);
		...
		if ($this->blk[$blvl]['border_right']) {
			...
			if (isset($tbd['s']) && $tbd['s']) {
				if (!$brset && $tbd['style'] != 'dotted' && $tbd['style'] != 'dashed') {
					$this->writer->write('q');
					$this->SetLineWidth(0);
					$this->writer->write(sprintf('%.3F %.3F m ', ($x0 + $w) * $this->scale_cons, ($this->h - ($y0)) * $this->scale_cons));
					$this->writer->write(sprintf('%.3F %.3F l ', ($x0 + $w - $border_right) * $this->scale_cons, ($this->h - ($y0 + $border_top)) * $this->scale_cons));
					$this->writer->write(sprintf('%.3F %.3F l ', ($x0 + $w - $border_right) * $this->scale_cons, ($this->h - ($y0 + $h - $border_bottom)) * $this->scale_cons));
					$this->writer->write(sprintf('%.3F %.3F l ', ($x0 + $w) * $this->scale_cons, ($this->h - ($y0 + $h)) * $this->scale_cons));
					$this->writer->write(' h W n '); // Ends path no-op & Sets the clipping path
				...
				if ($brBR_V && $brBR_H) {
					...
					if ($tbd['style'] == 'solid' || $tbd['style'] == 'double') {
						$s .= (sprintf('%.3F %.3F m ', ($x0 + $w - ($border_right / 2)) * $this->scale_cons, ($this->h - ($y0 + $h)) * $this->scale_cons)) . "\n";
					} else {
						$s .= (sprintf('%.3F %.3F m ', ($x0 + $w - ($border_right / 2)) * $this->scale_cons, ($this->h - ($y0 + $h - ($border_right / 2))) * $this->scale_cons)) . "\n";
					}
				}
				/* -- BORDER-RADIUS -- */
				if ($brTR_V && $brTR_H) {
					$s .= (sprintf('%.3F %.3F l ', ($x0 + $w - ($border_right / 2)) * $this->scale_cons, ($this->h - ($y0 + ($border_right / 2) + $brTR_V) ) * $this->scale_cons)) . "\n";
					$s .= ($this->_EllipseArc($x0 + $w - $brTR_H, $y0 + $brTR_V, $brTR_H - $border_right / 2, $brTR_V - $border_right / 2, 1, 1)) . "\n";
				} else { 				/* -- END BORDER-RADIUS -- */
					if ($tbd['style'] == 'solid' || $tbd['style'] == 'double') {
						$s .= (sprintf('%.3F %.3F l ', ($x0 + $w - ($border_right / 2)) * $this->scale_cons, ($this->h - ($y0) ) * $this->scale_cons)) . "\n";
					} else {
						$s .= (sprintf('%.3F %.3F l ', ($x0 + $w - ($border_right / 2)) * $this->scale_cons, ($this->h - ($y0 + ($border_right / 2)) ) * $this->scale_cons)) . "\n";
					}
				}
				$s .= 'S' . "\n";
				$this->writer->write($s);
		...
		// Set clipping path
		$s = ' q 0 w '; // Line width=0
		$s .= sprintf('%.3F %.3F m ', ($bgx0 + $brbgTL_H ) * $this->scale_cons, ($this->h - $bgy0) * $this->scale_cons); // start point TL before the arc
		/* -- BORDER-RADIUS -- */
		if ($brbgTL_H || $brbgTL_V) {
			$s .= $this->_EllipseArc($bgx0 + $brbgTL_H, $bgy0 + $brbgTL_V, $brbgTL_H, $brbgTL_V, 2); // segment 2 TL
		}
		/* -- END BORDER-RADIUS -- */
		$s .= sprintf('%.3F %.3F l ', ($bgx0) * $this->scale_cons, ($this->h - ($bgy1 - $brbgBL_V )) * $this->scale_cons); // line to BL
		/* -- BORDER-RADIUS -- */
		if ($brbgBL_H || $brbgBL_V) {
			$s .= $this->_EllipseArc($bgx0 + $brbgBL_H, $bgy1 - $brbgBL_V, $brbgBL_H, $brbgBL_V, 3); // segment 3 BL
		}
		/* -- END BORDER-RADIUS -- */
		$s .= sprintf('%.3F %.3F l ', ($bgx1 - $brbgBR_H ) * $this->scale_cons, ($this->h - ($bgy1)) * $this->scale_cons); // line to BR
		/* -- BORDER-RADIUS -- */
		if ($brbgBR_H || $brbgBR_V) {
			$s .= $this->_EllipseArc($bgx1 - $brbgBR_H, $bgy1 - $brbgBR_V, $brbgBR_H, $brbgBR_V, 4); // segment 4 BR
		}
		/* -- END BORDER-RADIUS -- */
		$s .= sprintf('%.3F %.3F l ', ($bgx1) * $this->scale_cons, ($this->h - ($bgy0 + $brbgTR_V)) * $this->scale_cons); // line to TR
		/* -- BORDER-RADIUS -- */
		if ($brbgTR_H || $brbgTR_V) {
			$s .= $this->_EllipseArc($bgx1 - $brbgTR_H, $bgy0 + $brbgTR_V, $brbgTR_H, $brbgTR_V, 1); // segment 1 TR
		}
		/* -- END BORDER-RADIUS -- */
		$s .= sprintf('%.3F %.3F l ', ($bgx0 + $brbgTL_H ) * $this->scale_cons, ($this->h - $bgy0) * $this->scale_cons); // line to TL
		// Box Shadow
		$shadow = '';
		if (isset($this->blk[$blvl]['box_shadow']) && $this->blk[$blvl]['box_shadow'] && $h > 0) {
			foreach ($this->blk[$blvl]['box_shadow'] as $sh) {
				...
				$shadow .= sprintf('0 %.3F m %.3F %.3F l ', $this->h * $this->scale_cons, $this->w * $this->scale_cons, $this->h * $this->scale_cons);
				$shadow .= sprintf('%.3F 0 l 0 0 l 0 %.3F l ', $this->w * $this->scale_cons, $this->h * $this->scale_cons);
				$shadow .= 'W n' . "\n";
				...
				// Shadow Offset
				if ($sh['x'] || $sh['y']) {
					$shadow .= sprintf(' q 1 0 0 1 %.4F %.4F cm', $sh['x'] * $this->scale_cons, -$sh['y'] * $this->scale_cons) . "\n";
				}
				...
				$shadow .= sprintf('%.3F %.3F m ', ($p1x ) * $this->scale_cons, ($this->h - ($p1y )) * $this->scale_cons);
				$shadow .= sprintf('%.3F %.3F %.3F %.3F %.3F %.3F c ', ($p1c2x) * $this->scale_cons, ($this->h - ($p1y)) * $this->scale_cons, ($p4x) * $this->scale_cons, ($this->h - ($p4c2y)) * $this->scale_cons, ($p4x) * $this->scale_cons, ($this->h - ($p4y)) * $this->scale_cons);
				$patch_array[0]['f'] = 0;
				...
				$shadow .= sprintf('%.3F %.3F l ', ($p3x ) * $this->scale_cons, ($this->h - ($p3y )) * $this->scale_cons);
				$patch_array[1]['f'] = 2;
				...
				$shadow .= sprintf('%.3F %.3F %.3F %.3F %.3F %.3F c ', ($p1x) * $this->scale_cons, ($this->h - ($p1c2y)) * $this->scale_cons, ($p4c2x) * $this->scale_cons, ($this->h - ($p4y)) * $this->scale_cons, ($p4x) * $this->scale_cons, ($this->h - ($p4y)) * $this->scale_cons);
				$patch_array[2]['f'] = 2;
		...
		if (isset($arrayaux['FONT-SIZE'])) {
			...
			if (is_numeric($v[0]) || ($v[0] === '.')) {
				if ($type == 'BLOCK' && $this->blklvl > 0 && isset($this->blk[$this->blklvl - 1]['InlineProperties']) && isset($this->blk[$this->blklvl - 1]['InlineProperties']['size'])) {
					...
					$mmsize = $this->sizeConverter->convert($v, $this->default_font_size / $this->scale_cons);
				} else {
					$mmsize = $this->sizeConverter->convert($v, $this->FontSize);
				}
				$this->SetFontSize($mmsize * ($this->scale_cons), false); // Get size in points (pt)
			} else {
				$v = strtoupper($v);
			...
			for ($j = 0; $j < $numcols; $j++) { // columns
				if (isset($cells[$i][$j]) && $cells[$i][$j]) {
					...
					if ($c['R'] && isset($c['textbuffer'])) {
						...
						if ($c['R'] >= 45 && $c['R'] < 90) {
							$tempch = ((sin(deg2rad($c['R']))) * $tempch ) + ((sin(deg2rad($c['R']))) * (($c['textbuffer'][0][11] / $this->scale_cons) / $this->shrin_k));
						}
						$this->SetFont($s_f, $s_st, $s_fs, true, true);
						$ch = ($tempch ) + $extrh;
					} else {
	...
	function _tableWrite(&$table, $split = false, $startrow = 0, $startcol = 0, $splitpg = 0, $rety = 0)
	{
		...
		if ($table['overflow'] == 'hidden' && $level == 1 && !$this->table_rotate && !$this->ColActive) {
			// Bounding rectangle to clip
			$this->tableClipPath = sprintf('q %.3F %.3F %.3F %.3F re W n', $x0 * $this->scale_cons, $this->h * $this->scale_cons, $this->blk[$this->blklvl]['inner_width'] * $this->scale_cons, -$this->h * $this->scale_cons);
			...
			foreach ($this->columnbuffer as $key => $s) {
				if (isset($s['rel_y'])) { // only process position sensitive data
					...
					if (isset($this->columnLinks[$s['col']][intval($s['x'])][intval($s['y'])])) {
						...
						$this->PageLinks[$this->page][$ref][0] += ($xadj * $this->scale_cons);
						$this->PageLinks[$this->page][$ref][1] -= ($yadj * $this->scale_cons);
				...
				foreach ($this->columnbuffer as $key => $s) {
					if (isset($s['rel_y'])) { // only process position sensitive data
						...
						if (($ratio > 1) && ($ratio <= $this->max_colH_correction)) {
							...
							if (isset($this->columnLinks[$s['col']][intval($s['x'])][intval($s['y'])])) {
								$ref = $this->columnLinks[$s['col']][intval($s['x'])][intval($s['y'])];
								$this->PageLinks[$this->page][$ref][1] -= ($yadj * $this->scale_cons); // y value
	...
	function columnAdjustPregReplace($type, $xadj, $yadj, $pattern, $subject)
	{
		...
		return str_replace($matches[0], $this->columnAdjustAdd($type, $this->scale_cons, $xadj, $yadj, $matches[1], $matches[2], $matches[3], $matches[4], $matches[5], $matches[6]), $subject);
	}
	/* -- END COLUMNS -- */
	...
	function printtablebuffer()
	{
		...
		foreach ($this->tbrot_Links as $p => $l) {
			...
			foreach ($l as $v) {
				...
				$w = $v[2] / $this->scale_cons;
				$h = $v[3] / $this->scale_cons;
				$ax = ($v[0] / $this->scale_cons) - $this->tbrot_x0;
				$ay = (($this->hPt - $v[1]) / $this->scale_cons) - $this->tbrot_y0;
				...
				$v[0] = $bx * $this->scale_cons;
				$v[1] = ($this->h - $by) * $this->scale_cons;
				$v[2] = $h * $this->scale_cons; // swap width and height
				$v[3] = $w * $this->scale_cons;
	...
	function Ellipse($x, $y, $rx, $ry, $style = 'S')
	{
		...
		$this->writer->write(sprintf('%.3F %.3F m %.3F %.3F %.3F %.3F %.3F %.3F c', ($x + $rx) * $this->scale_cons, ($h - $y) * $this->scale_cons, ($x + $rx) * $this->scale_cons, ($h - ($y - $ly)) * $this->scale_cons, ($x + $lx) * $this->scale_cons, ($h - ($y - $ry)) * $this->scale_cons, $x * $this->scale_cons, ($h - ($y - $ry)) * $this->scale_cons));
		$this->writer->write(sprintf('%.3F %.3F %.3F %.3F %.3F %.3F c', ($x - $lx) * $this->scale_cons, ($h - ($y - $ry)) * $this->scale_cons, ($x - $rx) * $this->scale_cons, ($h - ($y - $ly)) * $this->scale_cons, ($x - $rx) * $this->scale_cons, ($h - $y) * $this->scale_cons));
		$this->writer->write(sprintf('%.3F %.3F %.3F %.3F %.3F %.3F c', ($x - $rx) * $this->scale_cons, ($h - ($y + $ly)) * $this->scale_cons, ($x - $lx) * $this->scale_cons, ($h - ($y + $ry)) * $this->scale_cons, $x * $this->scale_cons, ($h - ($y + $ry)) * $this->scale_cons));
		$this->writer->write(sprintf('%.3F %.3F %.3F %.3F %.3F %.3F c %s', ($x + $lx) * $this->scale_cons, ($h - ($y + $ry)) * $this->scale_cons, ($x + $rx) * $this->scale_cons, ($h - ($y + $ly)) * $this->scale_cons, ($x + $rx) * $this->scale_cons, ($h - $y) * $this->scale_cons, $op));
	}
	...
	function WriteBarcode($code, $showtext = 1, $x = '', $y = '', $size = 1, $border = 0, $paddingL = 1, $paddingR = 1, $paddingT = 2, $paddingB = 2, $height = 1, $bgcol = false, $col = false, $btype = 'ISBN', $supplement = '0', $supplement_code = '', $k = 1)
	{
		...
		if (($btype === 'EAN13' && $showtext) || $btype === 'ISBN' || $btype === 'ISSN') {
			...
			$loop = 0;
			while ($loop == 0) {
				$this->SetFontSize($codestr_fontsize * 1.4 * $this->scale_cons, false); // don't write
			...
			$this->SetFont('', '', $codestr_fontsize * 1.4 * $this->scale_cons, true, true); // * 1.4 because font height is only 7/10 of given mm
			// WORD SPACING
			if ($fbwi > $sz) {
				$xtra = $fbwi - $sz;
				$charspacing = $xtra / (strlen($codestr) - 1);
				if ($charspacing) {
					$this->writer->write(sprintf('BT %.3F Tc ET', $charspacing * $this->scale_cons));
				}
			}
		...
		$this->SetFontSize(($outerfontsize / 3) * 3 * $fh * $size * $this->scale_cons); // 3mm numerals (FontSize is larger to account for space above/below characters)
		...
		$this->SetFontSize(3 * $fh * $size * $this->scale_cons); // 3mm numerals (FontSize is larger to account for space above/below characters)
	...
	function transformTranslate($t_x, $t_y, $returnstring = false)
	{
		...
		$tm[2] = 0;
		$tm[3] = 1;
		$tm[4] = $t_x * $this->scale_cons;
		$tm[5] = -$t_y * $this->scale_cons;
