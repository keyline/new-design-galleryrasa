...
if (isset($_POST['classification4'])) {
    ...
    $classification1arr = $_POST['classification4'];
...
if (isset($_POST['medium'])) {
    ...
    $mediumarr = $_POST['medium'];

    $cntmedium = count($mediumarr);

    $medi = 1;
    $mediumstr = '';
    $mediumsearchkey = ',Medium: ';
    foreach ($mediumarr as $mkey => $mval) {

        if ($medi == '1') {
            //$publicationstr .= "(f.attribute_name='va_year_publication' AND v.value like " . $pval . ")";
            $mediumstr .= '(f.attribute_name="va_medium" AND v.value ="' . $mval . '") ';
        } else {
            //$publicationstr .= " OR (f.attribute_name='va_year_publication' AND v.value like '" . $pval . "')";
            $mediumstr .= ' OR (f.attribute_name="va_medium" AND v.value ="' . $mval . '") ';
        }
        
        
        if ($cntmedium == $medi) {
            $mediumsearchkey .= $mval;
        } else {
            $mediumsearchkey .= $mval . ',';
        }
 ...
 FROM products_ecomc p 
  ...
  JOIN attr_common_flds_ecomc f ON v.attr_id = f.id 
 where 
 p.category_id = 19 and " . $mediumstr . " GROUP BY t.product_id
    )c
";
 ...
 p.category_id = 19 and " . $mediumstr . " GROUP BY t.product_id
        ...
        if (empty($finalParam) && $classificationflag == true) {
            $finalmediumstr2 = "where b.product_id  = c.product_id";
        } else if (!empty($finalParam) && $classificationflag == false) {
            $finalmediumstr2 = " where a.product_id  = c.product_id";
        } else if (!empty($finalParam) && $classificationflag == true) {
            $finalmediumstr2 = " and b.product_id  = c.product_id";
        } else {
            $finalmediumstr2 = "";
        }
    }

$keyword .= $mediumsearchkey;
...
if (empty($finalParam) && $classificationflag == false && $mediumflag == false) {
    ...
    if (($classificationflag == false) && ($mediumflag == false) && !empty($finalParam)) {
...
select " . $prodtag . "product_id from " .
                ...
                $finalclassificationstr1 . " " .
                $finalmediumstr1 . " 
 " . $finalclassificationstr2 . $finalmediumstr2 . " 
