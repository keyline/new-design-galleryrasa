...
acfe.id = ave.attr_id";
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        try {
            ...
            $sql = "SELECT * FROM products_ecomc p LEFT JOIN product_attribute_value pav ON p.prodid=pav.product_id
                LEFT JOIN attribute_value_ecomc av ON pav.attribute_value_id=av.attr_value_id LEFT JOIN attr_common_flds_ecomc af ON af.id=av.attr_id
                WHERE
                af.id=197 AND p.category_id=:categoryID ORDER BY prodid DESC";
            $q = $conn->prepare($sql);
            $category_id = 19;
        ...
        include(ADMIN_HTML . "admin-headerInc.php");
        include(ADMIN_HTML . 'archivist-remarks-tpl.php');
        include(ADMIN_HTML . "admin-footerInc.php");
    }
}