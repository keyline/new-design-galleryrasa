...
function sendJSONdata() {
    ...
    try {
        $conn = dbconnect();
        if (in_array("va-artwork", $data)) {
            $qry = "SELECT `va_image_id` AS m_image_id, `va_image_name` AS m_image_name, `va_image_category_text` AS m_image_category_text, `va_is_featured` AS is_featured FROM " . VISUALARCHIVE_IMAGES_TBL . " WHERE va_product_id=:productID AND va_image_category=:category ORDER by va_date_created";
            $q = $conn->prepare($qry);
            $bind = array(':productID' => stripcslashes($_POST['product']), ':category' => "AW");
            //echo PdoDebugger::show($qry, $bind);
        ...
        } elseif (in_array("va-artistphoto", $data)) {
            $qry = "SELECT `va_image_id` AS m_image_id, `va_image_name` AS m_image_name, `va_image_category_text` AS m_image_category_text, `va_is_featured` AS is_featured FROM " . VISUALARCHIVE_IMAGES_TBL . " WHERE va_product_id=:productID AND va_image_category=:category ORDER by va_date_created";
            $q = $conn->prepare($qry);
            $bind = array(':productID' => stripcslashes($_POST['product']), ':category' => "PA");
            //echo PdoDebugger::show($qry, $bind);
...
function DeleteImage() {
    ...
    try {
        /*  Choice of table because Visual Archive and Memorabilia Images are deleted from same window  */
        if (in_array("Artist Photograph", $deleteId) || in_array("Art Work", $deleteId)) {
            $table = "`visual_archive_images`";
            $param = "`va_image_id`=:imageID";
            
            
            

            $vaimgarr = get_va_singleimage(intval($deleteId[1]));

            $imgname = $vaimgarr['va_image_name'];

            $va_thumb_destination = '../' . 'product_images/artwork_thumbs/';
            $va_destination = '../' . '../' . 'Art Work/';

            $unlinkthumblink = realpath(__DIR__ . '/' . $va_thumb_destination . $imgname);
            ...
            //$unlinkrealimg = realpath(__DIR__ . $va_destination . $fileimage);
            
             $unlinkrealimg = realpath(__DIR__ . '/../' . '../' . 'Art Work/' . $fileimage);
            

            unlink($unlinkthumblink);

            unlink($unlinkrealimg);
            
            
            
            
        } else {
            $table = "`memorabilia_images`";
            $param = "`m_image_id`=:imageID";
        }
