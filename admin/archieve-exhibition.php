...
try {
    $conn = dbconnect();
    $err = false;
    $query1 = "UPDATE `exhibition` SET status = 1 WHERE id = ". $exhibition_id."";
    $q = $conn->prepare($query1);
    $result=$q->execute();
    if($result) {
        $_SESSION['succ'] = "Exhibition is archieved successfully";
        goto_location("exhibition-list.php");
    } else {
        $_SESSION['fail'] = "Exhibition is not archieved";
        goto_location("exhibition-list.php");
    }
