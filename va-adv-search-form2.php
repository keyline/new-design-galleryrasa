...
if (isset($_POST['classification2'])) {
    ...
    $classification1arr = $_POST['classification2'];
    ...
    if (empty($finalParam)) {
        ...
        $finalclassificationstr1 = " (SELECT t.product_id   
 ...
 where p.category_id = 19 and " . $classification1str . " GROUP BY t.product_id)b
";

        //$finalclassificationstr2 = " where a.product_id = b.product_id ";

        $finalclassificationstr2 = " ";
    } else {
...
$keyword .= $classificationsearchkey;
    ...
    //$finalclassificationstr2 = " a.product_id = b.product_id ";
} else {

    $finalclassificationstr1 = "";
    $finalclassificationstr2 = "";
...
if (isset($_POST['publicationyear'])) {
    ...
    //$finalpublicationstr2 = "and b.product_id  = c.product_id";
} else {
...
if (isset($_POST['frompublication']) && $_POST['frompublication'] != '' && isset($_POST['topublication']) && $_POST['topublication'] != '') {
    $frompublication = substr($_POST['frompublication'], 0, 4);

    $topublication = substr($_POST['topublication'], 0, 4);
    
    $pubyrrangekey = ",From: ".$frompublication.', To:'.$topublication;
    
    $keyword .= $pubyrrangekey;

    if ($publicationflag == true) {


        $nopublicationyrrangestr1 = " OR (f.attribute_name='va_year_publication' AND LEFT(v.value,4) between '" . $frompublication . "' and '" . $topublication . "') ";


        $finalpublicationrangestr1 = "";
        $finalpublicationrangestr2 = "";
        $yearrangeflag = false;
    } else {
        $yearrangeflag = true;

        $nopublicationyrrangestr1 = "";


        $pubyrrangestr = "(f.attribute_name='va_year_publication' AND LEFT(v.value,4) between '" . $frompublication . "' and '" . $topublication . "')";

        if (empty($finalParam) && $classificationflag == false && $publicationflag == false) {
 ...
 FROM products_ecomc p 
  ...
  JOIN attr_common_flds_ecomc f ON v.attr_id = f.id 
 where 
 p.category_id = 19 and " . $pubyrrangestr . " GROUP BY t.product_id
    )d
";
...
";
        ...
        if (empty($finalParam) && $classificationflag == false && $publicationflag == true) {
            $finalpublicationrangestr2 = "where c.product_id  = d.product_id";
        } else if (!empty($finalParam) && $classificationflag == false && $publicationflag == false) {
            $finalpublicationrangestr2 = " where a.product_id  = d.product_id";
        } else if (!empty($finalParam) && $classificationflag == false && $publicationflag == true) {
            $finalpublicationrangestr2 = " and c.product_id  = d.product_id";
        } else if (empty($finalParam) && $classificationflag == true && $publicationflag == false) {
            $finalpublicationrangestr2 = " where b.product_id  = d.product_id";
        } else if (!empty($finalParam) && $classificationflag == true && $publicationflag == false) {
            $finalpublicationrangestr2 = " and b.product_id  = d.product_id";
        } else if (!empty($finalParam) && $classificationflag == true && $publicationflag == true) {
            $finalpublicationrangestr2 = " and c.product_id  = d.product_id";
        } else if (empty($finalParam) && $classificationflag == true && $publicationflag == true) {
            $finalpublicationrangestr2 = " and c.product_id  = d.product_id";
        } else {
...
if (isset($_POST['publicationyear'])) {
    ...
    $publicationarr = $_POST['publicationyear'];

    $cntpublication = count($publicationarr);

    $pubi = 1;
    $publicationstr = '';
    $publicationsearchkey = ',Year of Publication: ';
    foreach ($publicationarr as $pkey => $pval) {

        if ($pubi == '1') {
            //$publicationstr .= "(f.attribute_name='va_year_publication' AND v.value like " . $pval . ")";
            $publicationstr .= '(f.attribute_name="va_year_publication" AND v.value like "%' . $pval . '%") ' . $nopublicationyrrangestr1;
        } else {
            //$publicationstr .= " OR (f.attribute_name='va_year_publication' AND v.value like '" . $pval . "')";
            $publicationstr .= ' OR (f.attribute_name="va_year_publication" AND v.value like "%' . $pval . '%") ' . $nopublicationyrrangestr1;
        }
        
        
        if ($cntpublication == $pubi) {
            $publicationsearchkey .= $pval;
        } else {
            $publicationsearchkey .= $pval . ',';
        }
 ...
 FROM products_ecomc p 
  ...
  JOIN attr_common_flds_ecomc f ON v.attr_id = f.id 
 where 
 p.category_id = 19 and " . $publicationstr . " GROUP BY t.product_id
    )c
";

        //$finalpublicationstr2 = "and b.product_id  = c.product_id";

        $finalpublicationstr2 = "";
    } else {
        $finalpublicationstr1 = " , (
    SELECT t.product_id   
 FROM products_ecomc p 
 ...
 p.category_id = 19 and " . $publicationstr . " GROUP BY t.product_id
        ...
        if (empty($finalParam) && $classificationflag == true) {
            $finalpublicationstr2 = "where b.product_id  = c.product_id";
        } else if (!empty($finalParam) && $classificationflag == false) {
            $finalpublicationstr2 = " where a.product_id  = c.product_id";
        } else if (!empty($finalParam) && $classificationflag == true) {
            $finalpublicationstr2 = " and b.product_id  = c.product_id";
        } else {
            $finalpublicationstr2 = "";
        }
...
if (!empty($finalParam)) {
 ...
 and " . $finalParam . " GROUP BY t.product_id)a ";
} else {
    $artistsrt = "";
}
...
if (!empty($finalParam)) {
    $prodtag = "a.";
} else if ($classificationflag == true) {
    $prodtag = "b.";
} else if ($publicationflag == true) {
    $prodtag = "c.";
} else if ($yearrangeflag == true) {
    $prodtag = "d.";
} else {
    $prodtag = "";
}

if (empty($finalParam) && $classificationflag == false && $publicationflag == false && $yearrangeflag == false) {
    ...
    if (($classificationflag == false) && ($publicationflag == false) && ($yearrangeflag == false) && !empty($finalParam)) {
        ...
        $sql = 'SELECT
                    ...
                    LEFT JOIN attribute_value_ecomc v ON t.attribute_value_id = v.attr_value_id
                    LEFT JOIN attr_common_flds_ecomc f ON v.attr_id = f.id
                    WHERE p.category_id = 19 AND ' . $finalParam . ' GROUP BY t.product_id) GROUP BY t.attribute_value_id order by p.prodname';
...
(
select " . $prodtag . "product_id from " .
                $artistsrt . " " .
                $finalclassificationstr1 . " " .
                $finalpublicationstr1 . " " .
                $finalpublicationrangestr1 . " 
 " . $finalclassificationstr2 . $finalpublicationstr2 . $finalpublicationrangestr2 . " 
