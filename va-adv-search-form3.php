...
if (isset($_POST['classification3'])) {
    ...
    $classification1arr = $_POST['classification3'];
...
if (isset($_POST['fromartworkyr']) && $_POST['fromartworkyr'] != '' && isset($_POST['toartworkyr']) && $_POST['toartworkyr'] != '') {
    $fromartwork = substr($_POST['fromartworkyr'], 0, 4);

    $toartwork = substr($_POST['toartworkyr'], 0, 4);
    
    $artyrrangekey = ",From: ".$fromartwork.', To:'.$toartwork;
    
    $keyword .= $artyrrangekey;

    if ($artworkflag == true) {


        $noartworkyrrangestr1 = " OR (f.attribute_name='va_year_artwork' AND LEFT(v.value,4) between '" . $fromartwork . "' and '" . $toartwork . "') ";


        $finalartworkrangestr1 = "";
        $finalartworkrangestr2 = "";
        $yearrangeflag = false;
    } else {
        $yearrangeflag = true;

        $noartworkyrrangestr1 = "";


        $artyrrangestr = "(f.attribute_name='va_year_artwork' AND LEFT(v.value,4) between '" . $fromartwork . "' and '" . $toartwork . "')";

        if (empty($finalParam) && $classificationflag == false && $artworkflag == false) {
 ...
 FROM products_ecomc p 
  ...
  JOIN attr_common_flds_ecomc f ON v.attr_id = f.id 
 where 
 p.category_id = 19 and " . $artyrrangestr . " GROUP BY t.product_id
    )d
";
...
";
        ...
        if (empty($finalParam) && $classificationflag == false && $artworkflag == true) {
            $finalartworkrangestr2 = "where c.product_id  = d.product_id";
        } else if (!empty($finalParam) && $classificationflag == false && $artworkflag == false) {
            $finalartworkrangestr2 = " where a.product_id  = d.product_id";
        } else if (!empty($finalParam) && $classificationflag == false && $artworkflag == true) {
            $finalartworkrangestr2 = " and c.product_id  = d.product_id";
        } else if (empty($finalParam) && $classificationflag == true && $artworkflag == false) {
            $finalartworkrangestr2 = " where b.product_id  = d.product_id";
        } else if (!empty($finalParam) && $classificationflag == true && $artworkflag == false) {
            $finalartworkrangestr2 = " and b.product_id  = d.product_id";
        } else if (!empty($finalParam) && $classificationflag == true && $artworkflag == true) {
            $finalartworkrangestr2 = " and c.product_id  = d.product_id";
        } else if (empty($finalParam) && $classificationflag == true && $artworkflag == true) {
            $finalartworkrangestr2 = " and c.product_id  = d.product_id";
        } else {
...
if (isset($_POST['artworkyear'])) {
    ...
    $artworkarr = $_POST['artworkyear'];

    $cntartwork = count($artworkarr);

    $arti = 1;
    $artworkstr = '';
    $artworksearchkey = ',Year of Artwork: ';
    foreach ($artworkarr as $akey => $aval) {

        if ($arti == '1') {
            //$publicationstr .= "(f.attribute_name='va_year_publication' AND v.value like " . $pval . ")";
            $artworkstr .= '(f.attribute_name="va_year_artwork" AND v.value like "%' . $aval . '%") ' . $noartworkyrrangestr1;
        } else {
            //$publicationstr .= " OR (f.attribute_name='va_year_publication' AND v.value like '" . $pval . "')";
            $artworkstr .= ' OR (f.attribute_name="va_year_artwork" AND v.value like "%' . $aval . '%") ' . $noartworkyrrangestr1;
        }
        
        
        if ($cntartwork == $arti) {
            $artworksearchkey .= $aval;
        } else {
            $artworksearchkey .= $aval . ',';
        }
 ...
 FROM products_ecomc p 
  ...
  JOIN attr_common_flds_ecomc f ON v.attr_id = f.id 
 where 
 p.category_id = 19 and " . $artworkstr . " GROUP BY t.product_id
    )c
";
 ...
 p.category_id = 19 and " . $artworkstr . " GROUP BY t.product_id
        ...
        if (empty($finalParam) && $classificationflag == true) {
            $finalartworkstr2 = "where b.product_id  = c.product_id";
        } else if (!empty($finalParam) && $classificationflag == false) {
            $finalartworkstr2 = " where a.product_id  = c.product_id";
        } else if (!empty($finalParam) && $classificationflag == true) {
            $finalartworkstr2 = " and b.product_id  = c.product_id";
        } else {
            $finalartworkstr2 = "";
        }
    }

$keyword .= $artworksearchkey;
...
if (empty($finalParam) && $classificationflag == false && $artworkflag == false && $yearrangeflag == false) {
    ...
    if (($classificationflag == false) && ($artworkflag == false) && ($yearrangeflag == false) && !empty($finalParam)) {
...
select " . $prodtag . "product_id from " .
                ...
                $finalartworkstr1 . " " .
                $finalartworkrangestr1 . " 
 " . $finalclassificationstr2 . $finalartworkstr2 . $finalartworkrangestr2 . " 
